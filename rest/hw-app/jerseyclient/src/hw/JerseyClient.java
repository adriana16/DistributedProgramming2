...
public class JerseyClient {
  public static void main(String args[]) {
    ...
    switch(tip){
      case "plain":  
        webTarget=client.target("http://localhost:8080/HelloWorld/webresources/helloworld");
        invocationBuilder=webTarget.request();
        response=invocationBuilder.get(String.class);
        break;
      case "html": 
        webTarget=client.target("http://localhost:8080/HelloWorld/webresources/helloworld/html");
        invocationBuilder=webTarget.request();//(MediaType.TEXT_PLAIN_TYPE);
        response=invocationBuilder.get(String.class);
        break;
      case "xml": 
        webTarget=client.target("http://localhost:8080/HelloWorld/webresources/helloworld/xml");
        invocationBuilder=webTarget.request();//(MediaType.TEXT_PLAIN_TYPE);
        response=invocationBuilder.get(String.class);
        break;
    }   
